...
export async function POST(req: NextRequest) {
  ...
  // ✅ FIX #1: Validate document existence and type
  if (!document || !(document instanceof Blob)) {
    return NextResponse.json(
      { success: false, error: "Invalid or missing PDF file" },
      { status: 400 }
    );
  }

  // ✅ FIX #2: Optional - PDF size limit
  if (document.size > 10 * 1024 * 1024) {
    return NextResponse.json(
      { success: false, error: "PDF too large (max 10MB)" },
      { status: 413 }
    );
  }
  ...
  try {
    ...
    if (!process.env.OPENAI_API_KEY) {
      // ✅ FIX #3: Use unified response shape
      return NextResponse.json(
        { success: false, error: "OpenAI Key not provided" },
        { status: 500 }
      );
    }
